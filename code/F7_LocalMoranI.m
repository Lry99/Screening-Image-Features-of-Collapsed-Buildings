...
%% calculate Local Moran'I of a sample
function MoranI = NMoranI(raster)

[n1,n2] = size(raster);
neighbour_MoranI = zeros(n1,n2);
half = 1;
...
for i = half+1:n1-half
    for j = half+1:n2-half
        ...
        if isempty(find(isnan(windw), 1))
            neighbour_MoranI(i,j) = one_nei_MoranI(windw);
        else
            neighbour_MoranI(i,j) = nan;
        end
    end
end
MoranI = nanmean(neighbour_MoranI(half+1:n1-half,half+1:n2-half),'all');
end

function I = one_nei_MoranI(windw)

Wa = sqrt(2)/(4*(sqrt(2)+1));
Wb = 1/(4*(sqrt(2)+1));
w = [Wb Wa Wb;Wa 0 Wa;Wb Wa Wb];

Xi = windw(5);
X_bar = mean(windw(:));
S2 = var(windw(:));

I = (Xi - X_bar)/S2 * sum(w.*(windw-X_bar),'all');
