...
parfor ppta = 1:len_sample
   ...
   pre_fs(ppta) = Localmean(pre_gray,s_win);
   ...
   post_fs(ppta) = Localmean(post_gray,s_win);
end
t2=clock;
t=etime(t2,t1);
...
function mean_neighbour_mean = Localmean(raster,s_window)

[n1,n2] = size(raster);
half = (s_window-1)/2;
neighbour_mean = zeros(n1,n2);

for i = half+1:n1-half
    for j = half+1:n2-half
        windw = raster(i-half:i+half,j-half:j+half);
        if isempty(find(isnan(windw), 1))
            neighbour_mean(i,j) = mean(windw(:));
        else
            neighbour_mean(i,j) = nan;
        end
    end
end
mean_neighbour_mean = nanmean(neighbour_mean(half+1:n1-half,half+1:n2-half),'all');
end
