...
for s_win = s_wins
    ...
    parfor ppta = 1:len_sample
        ...
        pre_fs(ppta) = Localstd(pre_gray,s_win);
        ...
        post_fs(ppta) = Localstd(post_gray,s_win);
    end
    t2=clock;
    t=etime(t2,t1);
...
%% calculate Local std
function mean_neighbour_std = Localstd(raster,s_window)

[n1,n2] = size(raster);
half = (s_window-1)/2;
neighbour_std = zeros(n1,n2);

for i = half+1:n1-half
    for j = half+1:n2-half
        windw = raster(i-half:i+half,j-half:j+half);
        if isempty(find(isnan(windw), 1))
            neighbour_std(i,j) = std(windw(:));
        else
            neighbour_std(i,j) = nan;
        end
    end
end
mean_neighbour_std = nanmean(neighbour_std(half+1:n1-half,half+1:n2-half),'all');
end
