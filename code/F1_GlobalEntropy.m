...
parfor ppta = 1:len_sample
   ...
   pre_valid = pre_gray(pre_gray~=0);
   pre_fs(ppta) = comentropy(pre_valid);


   post1 = load([sample_dir,'\',post_files(ppta).name]);
   post = post1.post;
   post_gray = double(rgb2gray(uint8(post)));
   post_valid = post_gray(post_gray~=0);
   post_fs(ppta) = comentropy(post_valid);
end
t2=clock;
t=etime(t2,t1);
