...
for s_win = s_wins
    for level = levels
        ...
        parfor ppta = 1:len_sample
            ...
            pre_fs(ppta) = m_GLCM_Correlation(pre_gray,s_win,level);
            ...
            post_fs(ppta) = m_GLCM_Correlation(post_gray,s_win,level);
        end
        t2=clock;
        t=etime(t2,t1);
...
function mean_GLCM = m_GLCM_Correlation(raster,s_win,level)
...
for i = half+1:n1-half
    for j = half+1:n2-half-1
        ...
        if isempty(find(isnan(windw), 1))
            neighbour_GLCM(i,j) = GLCM_Correlation(windw,level);
        else
...
function a = GLCM_Correlation(win,bin)
